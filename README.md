...
- 디스크 캐시<br>
<a href="https://developer.android.com/topic/libraries/architecture/paging/v3-network-db?hl=ko">RemoteMediator</a>를 활용해 로컬DB 기반의 Pagination을 지원하는 커스텀 디스크 캐싱을 구현했습니다. RemoteMediator는 서버 데이터를 페이지 단위로 가져오면서, 로컬DB에 저장하여 오프라인에서도 데이터를 유지할 수 있도록 하는 Google에서 제공하는 기술입니다. 이 기술에 아이디어를 추가해서 효율적인 데이터 호출과 함께, 로컬DB로 디스크 캐시 효과를 낼 수 있지 않을까 생각하여 도전했습니다. '캐시'라는 정체성을 명확히 하기 위해서, LRU 알고리즘 캐싱 정책과 유사하게 로컬 DB에 캐시 유효기간을 24시간으로 정했고, 사용되지 않은 순서 삭제 정책을 적용했습니다.<br>
- 메모리 캐시<br>
Glide 라이브러리를 활용하여 이미지 로딩 및 메모리 캐시를 지원합니다.
- 화면 회전<br>
회면 회전에 대응하여 세로 모드에서는 2열, 가로 모드에서는 4열로 썸네일을 표시하도록 레이아웃을 동적으로 조정했습니다.
